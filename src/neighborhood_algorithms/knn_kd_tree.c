...
static void kdtree_knearest(KDTree *tree, const KDNode *actual_node, int depth, size_t point_index, size_t neighbours_index[K], double neighbours_distances[K])
{
	...
	const KDNode *next_node = update_to_next_node(actual_node, tree->pts, point_index, depth);
    ...
    if(plane_distance <= neighbours_distances[K-1]){ // Compare with the last best distance
        next_node = update_to_oposite_next_node(actual_node, tree->pts, point_index, depth);
		kdtree_knearest(tree, next_node, depth+1, point_index, neighbours_index, neighbours_distances);
    }
}
