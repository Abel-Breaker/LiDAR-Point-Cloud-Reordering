...
#pragma unroll
	for (index_t i = 0; i + 7 < window; i += 8) {
		...
		// Loads + differences
		__m512d xr = _mm512_sub_pd(_mm512_load_pd(xs + i), x0_vector);
		__m512d yr = _mm512_sub_pd(_mm512_load_pd(ys + i), y0_vector);
		__m512d zr = _mm512_sub_pd(_mm512_load_pd(zs + i), z0_vector);

		// Squared Euclidean distance
		__m512d result_vec = _mm512_fmadd_pd(zr, zr, _mm512_fmadd_pd(yr, yr, _mm512_mul_pd(xr, xr)));
