...
void check_neighborhoods_tfg(const struct matrix_t *matrix)
{
	...
	for (size_t i = 0; i < ITER; ++i) {
		...
		RadiusResult res = {};
		tfg_radius_search(matrix->points, index, matrix->bandwith_left[get_block_index(index, matrix->points->num_points)], matrix->bandwith_right[get_block_index(index, matrix->points->num_points)], &res);
...
void check_neighborhoods_tfg_opt(const struct matrix_t *matrix)
{
	...
	for (size_t i = 0; i < ITER; ++i) {
		...
		RadiusResult res = {};
		tfg_radius_search_opt(matrix->points, index, matrix->bandwith_left[get_block_index(index, matrix->points->num_points)], matrix->bandwith_right[get_block_index(index, matrix->points->num_points)], &res);
