...
static size_t partition_lomuto(KDTreePrune *tree, size_t start, size_t end, int axis)
{
...
static void nth_element(KDTreePrune *tree, size_t start, size_t nth, size_t end, int axis)
{
    while (end - start > 1) {
...
static KDNodePrune *kd_build(KDTreePrune *tree, size_t start, size_t end, int depth)
{
    // 1. Caso base
    ...
    // 5. Nodo del pool
    KDNodePrune *node = &tree->nodes[mid];
...
void create_kd_tree_prune(struct KDTreePrune *tree, const Points *pts)
{
    ...
    tree->max_distance = malloc(pts->num_points * sizeof(*(tree->max_distance)));
	tree->indices = malloc(pts->num_points * sizeof(*(tree->indices)));
	for (size_t i = 0; i < num_points; i++) {
		tree->indices[i] = i;
        tree->max_distance[i] = INFINITY;
	}
...
void destroy_kd_tree_prune(struct KDTreePrune *tree)
{
    ...
    free(tree->indices);
    free(tree->nodes);
    free(tree->max_distance);
}

static int verify_kd_node(KDTreePrune *tree, size_t start, size_t end, int depth)
{
...
static size_t count_nodes(KDNodePrune *node)
{
